...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 26 11:17:47 2025
...
def compare_layer_gradcams(model, rgb_loader, texture_loader, device='cuda', num_classes=3, num_samples=3, seed=42):
    ...
    # Prepare datasets
    rgb_dataset = rgb_loader.dataset
    class_names = rgb_dataset.classes
    
    # Persistent file to store indices
    indices_file = os.path.join(os.path.dirname(__file__), 'fixed_image_indices1.txt')
    
    # Function to select and save consistent indices
    def select_and_save_indices():
        selected_indices = []
        for label in range(num_classes):
            # Find all indices for this class
            class_indices = [idx for idx, (_, img_label) in enumerate(rgb_dataset) if img_label == label]
            
            # Use a deterministic method to select an image
            # Select the median index for each class to ensure consistency
            if len(class_indices) > 0:
                selected_index = sorted(class_indices)[len(class_indices) // 2]
    ...
    # Try to read existing indices, or create new ones
    try:
        with open(indices_file, 'r') as f:
            selected_indices = [int(line.strip()) for line in f.readlines()]
            # Verify we have exactly num_classes indices
            if len(selected_indices) != num_classes:
                selected_indices = select_and_save_indices()
    except (FileNotFoundError, ValueError):
        selected_indices = select_and_save_indices()
    
    # Collect images and labels based on selected indices
    selected_images = [rgb_dataset[idx][0] for idx in selected_indices]
    selected_labels = [rgb_dataset[idx][1] for idx in selected_indices]
    
    # Convert to tensors and add batch dimension
    rgb_inputs = torch.stack(selected_images).to(device)
    labels = torch.tensor(selected_labels).to(device)
    
    # Prepare matching texture inputs
    texture_dataset = texture_loader.dataset
    texture_inputs = torch.stack([texture_dataset[rgb_dataset.classes.index(class_name)][0] for class_name in class_names]).to(device)
    ...
    # Generate Grad-CAMs for layer4
    layer4_cams = generate_gradcam(model, rgb_inputs, texture_inputs, model.rgb_resnet.layer4)
    
    # Visualize comparisons
    fig, axes = plt.subplots(num_samples, 3, figsize=(15, 4*num_samples))
    fig.suptitle("Grad-CAM Visualizations from Layer 4 (One Image per Class)", fontsize=16)
...
if __name__ == "__main__":
    ...
    visualize_gradcam(model, rgb_loader, texture_loader, device, num_samples=3, layer='layer2')
